...
function warehouseRouteSlug(warehouse: WarehouseSite) {
  return slugifyText(
    `${warehouse.warehouseName}-${warehouse.dbId ?? warehouse.id}`,
  );
}
...
function warehouseMatchesRouteSlug(
  warehouse: WarehouseSite,
  slug: string | undefined,
) {
  if (!slug) return true;
  return (
    warehouseRouteSlug(warehouse) === slug ||
...
function hasWarehouseTag(delivery: Delivery) {
  return Boolean(
    ...
    delivery.toWarehouseId ||
    delivery.fromWarehouseName ||
    delivery.toWarehouseName,
  );
}

function deliveryMatchesWarehouse(
  delivery: Delivery,
  warehouse: WarehouseSite,
) {
  if (delivery.fromWarehouseId && delivery.fromWarehouseId === warehouse.dbId)
    return true;
  if (delivery.toWarehouseId && delivery.toWarehouseId === warehouse.dbId)
    return true;
...
function deliveryStatusFromApi(status: string | undefined): DeliveryStatus {
  ...
  if (
    ["accepted", "approved", "collected", "delivered", "received"].includes(
      normalized,
    )
  )
    return "delivered";
  if (["in_transit", "arrived", "dispatched", "released"].includes(normalized))
    return "dispatched";
  return "pending";
}
...
function deliveryItemsFromApi(delivery: ApiDelivery): DeliveryItem[] {
  ...
  return [
    ...
    {
      itemType: "amplifier",
      qty: toNumber(delivery.total_amplifier),
      unit: "pcs",
    },
    {
      itemType: "extender",
      qty: toNumber(delivery.total_extender),
      unit: "pcs",
    },
    ...
    {
      itemType: "ps_housing",
      qty: toNumber(delivery.total_psu_case),
      unit: "pcs",
    },
  ]
    .filter((item) => item.qty > 0)
...
function movementTypeFromApi(
  delivery: ApiDelivery,
  fromWarehouse: ApiWarehouse | null,
  toWarehouse: ApiWarehouse | null,
): MovementType {
  const rawType = String(
    delivery.movement_type ?? delivery.type ?? "",
  ).toLowerCase();
...
function mapApiDeliveries(apiDeliveries: ApiDelivery[]): Delivery[] {
  return apiDeliveries.map((delivery) => {
    const fromWarehouse =
      delivery.from_warehouse ?? delivery.fromWarehouse ?? null;
    ...
    const movementType = movementTypeFromApi(
      delivery,
      fromWarehouse,
      toWarehouse,
    );
    ...
    const subconName =
      toWarehouse?.subcontractor?.name ||
    ...
    const siteName =
      toWarehouse?.name ||
      (delivery.team?.name
        ? `${delivery.team.name} collected materials`
        : "") ||
      (delivery.span_count
        ? `${delivery.span_count} collected span deliveries`
        : "") ||
    ...
    return {
      ...
      source:
        delivery.total_cable !== undefined || delivery.span_count !== undefined
          ? "Field collections"
...
function mapApiWarehousesToSites(
  apiWarehouses: ApiWarehouse[],
  apiDeliveries: ApiDelivery[],
): WarehouseSite[] {
  return apiWarehouses.map((warehouse) => {
    const stocks = warehouse.stocks ?? [];
    const capacityUsed = stocks.reduce(
    ...
    const capacityMax =
      savedCapacity > 0
        ? savedCapacity
        : Math.max(100, Math.ceil(capacityUsed * 1.25));
    const percent =
      capacityMax > 0 ? Math.round((capacityUsed / capacityMax) * 100) : 0;
    const lowStockItems = stocks.filter(
      (stock) =>
        toNumber(stock.quantity) <= stockMeta(stock.item_type).threshold,
    ).length;
    ...
    const openDispatches = apiDeliveries.filter((delivery) => {
      const fromWarehouseId =
        delivery.from_warehouse_id ??
        delivery.from_warehouse?.id ??
        delivery.fromWarehouse?.id;
      const toWarehouseId =
        delivery.to_warehouse_id ??
        delivery.to_warehouse?.id ??
        delivery.toWarehouse?.id;
      return (
        isOpenDelivery(delivery.status) &&
        (fromWarehouseId === warehouse.id || toWarehouseId === warehouse.id)
      );
    }).length;
    ...
    return {
      ...
      dbId: warehouse.id,
      subconName:
        warehouse.subcontractor?.name ||
        warehouse.subcontractor?.company ||
        (warehouse.type === "main"
          ? "TelcoVantage"
          : "Unassigned Subcontractor"),
      warehouseName: warehouse.name || "Unnamed Warehouse",
      area:
        warehouse.address ||
        warehouse.subcontractor?.address ||
...
function makeWarehouseStockRow(
  warehouse: WarehouseSite,
  itemType: string,
...
function mapApiWarehousesToWarehouseStocks(
  apiWarehouses: ApiWarehouse[],
  deliveries: Delivery[],
): WarehouseStockItem[] {
  ...
  warehouseSites.forEach((warehouse) => {
    const apiWarehouse = apiWarehouses.find(
      (item) => item.id === warehouse.dbId,
    );
    ...
    (apiWarehouse?.stocks ?? []).forEach((stock) => {
      ...
      const row =
        rows.get(key) ?? makeWarehouseStockRow(warehouse, itemType, stock.unit);
      const qty = toNumber(stock.quantity);
      row.stockQty += qty;
    ...
    deliveries
      .filter(
        (delivery) =>
          ...
          deliveryMatchesWarehouse(delivery, warehouse),
      )
      .forEach((delivery) => {
        delivery.items.forEach((item) => {
          ...
          const row =
            rows.get(key) ??
            makeWarehouseStockRow(warehouse, item.itemType, item.unit);
...
function mapPreviewWarehouseStocks(
  ...
  return warehouses.flatMap((warehouse) => {
    const baseRows = stocks.map((stock) => ({
      ...
      stockQty: Math.max(
        0,
        Math.round(stock.qty / Math.max(warehouses.length, 1)),
      ),
      acceptedQty: 0,
...
const INITIAL_STOCKS: StockItem[] = [
  ...
  {
    id: "S2",
    name: "Utility Poles (Steel/Concrete)",
    category: "Structures",
    qty: 180,
    unit: "pcs",
    threshold: 30,
...
const INITIAL_DELIVERIES: Delivery[] = [
  {
    ...
    items: [
      {
        itemName: "Fiber Cable Drums (1000m)",
        itemType: "cable",
        qty: 5,
        unit: "drums",
      },
  ...
  },
  {
    ...
    items: [
      {
        itemName: "Utility Poles (Steel/Concrete)",
        itemType: "pole",
        qty: 40,
        unit: "pcs",
      },
...
export default function WarehouseInventory() {
  ...
  const userRoleLabel =
    warehouseUser?.role_name || warehouseUser?.role || "Staff";
  ...
  const handleTabChange = (tab: "stocks" | "deliveries") => {
    setSearchQuery("");
    const path = subconSlug
      ? `/warehouse/${subconSlug}/inventory`
      : "/warehouse/inventory";
    navigate(tab === "deliveries" ? `${path}?tab=deliveries` : path, {
      replace: true,
    });
  };
  ...
  const effectiveSelectedDeliveryId = useMemo(() => {
    if (
      selectedDeliveryId &&
      deliveries.some((d) => d.id === selectedDeliveryId)
    ) {
      return selectedDeliveryId;
    }
    const defaultDelivery =
      deliveries.find((d) => d.status === "dispatched") ||
  ...
  const deliveryCounts = useMemo(() => {
    return deliveries.reduce(
      ...
      { total: 0, pending: 0, dispatched: 0, delivered: 0, units: 0 } as Record<
        DeliveryStatus | "total" | "units",
        number
      >,
    );
  }, [deliveries]);

  const selectedDeliveryUnits = selectedDelivery
    ? getDeliveryUnits(selectedDelivery)
    : 0;
  const selectedDeliveryProgress = selectedDelivery
    ? DELIVERY_PROGRESS[selectedDelivery.status]
    : 0;
  const selectedDeliveryMarker = Math.min(
    92,
    Math.max(8, selectedDeliveryProgress),
  );
  const selectedDeliveryMeta = selectedDelivery
    ? DELIVERY_STATUS_META[selectedDelivery.status]
    : null;
  ...
  const handleUpdateDeliveryStatus = (
    ...
    if (
      ...
      const targetWarehouse =
        warehouseSites.find((warehouse) =>
          deliveryMatchesWarehouse(previousDelivery, warehouse),
        ) || selectedWarehouse;

      if (targetWarehouse) {
        previousDelivery.items.forEach((item) => {
          const key = `${warehouseRouteSlug(targetWarehouse)}-${item.itemType}`;
          const existing = nextWarehouseStocks.find(
            (stock) => stock.id === key,
          );
          ...
          const created = makeWarehouseStockRow(
            targetWarehouse,
            item.itemType,
            item.unit,
          );
  ...
  const loadData = async (silent = false, force = false) => {
    ...
    const cacheDeliveries = cacheGet<Delivery[]>(
      "warehouse_deliveries_list_v2",
    );
    const cacheWarehouses = cacheGet<WarehouseSite[]>(
      "warehouse_sites_list_v2",
    );
    const cacheWarehouseStocks = cacheGet<WarehouseStockItem[]>(
      "warehouse_stocks_by_site_v2",
    );
    ...
    if (!force && cacheStocks && cacheDeliveries && cacheWarehouses) {
      ...
      setWarehouseStocks(
        cacheWarehouseStocks ||
          mapPreviewWarehouseStocks(
            cacheWarehouses,
            cacheStocks,
            cacheDeliveries,
          ),
    ...
    try {
      ...
      const warehousesPayload = (await warehousesRes.json()) as
        | ApiWarehouse[]
        | PaginatedResponse<ApiWarehouse>;
      const deliveriesPayload = deliveriesRes.ok
        ? ((await deliveriesRes.json()) as
            | ApiDelivery[]
      ...
      const nextDeliveries = apiDeliveries.length
        ? mapApiDeliveries(apiDeliveries)
        : INITIAL_DELIVERIES;

      const nextWarehouses = apiWarehouses.length
        ? mapApiWarehousesToSites(apiWarehouses, apiDeliveries)
        : WAREHOUSE_PREVIEW;
      const nextStocks = apiWarehouses.length
        ? mapApiWarehousesToStocks(apiWarehouses)
        : INITIAL_STOCKS;
  ...
  const persistMovementRequest = async (delivery: Delivery) => {
    try {
      ...
      console.warn(
        "Unable to sync warehouse movement request yet; keeping local cache record.",
        err,
      );
    }
  };
  ...
  const selectedWarehouse = subconSlug
    ? warehouseSites.find((warehouse) =>
        warehouseMatchesRouteSlug(warehouse, subconSlug),
      ) || null
    : null;
  ...
  const selectedWarehouseStocks = useMemo(() => {
    ...
    return warehouseStocks.filter((item) => {
      if (selectedWarehouse.dbId && item.warehouseId === selectedWarehouse.dbId)
        return true;
      return (
        item.warehouseSlug === selectedSlug ||
        slugifyText(item.warehouseName) ===
          slugifyText(selectedWarehouse.warehouseName)
      );
    });
  ...
  const selectedWarehouseStats = useMemo(() => {
    const acceptedDeliveries = selectedWarehouseDeliveries.filter(
      (delivery) => delivery.status === "delivered",
    );
    const openRequests = selectedWarehouseDeliveries.filter(
      (delivery) => delivery.status !== "delivered",
    );
    return {
      currentUnits: selectedWarehouseStocks.reduce(
        (sum, item) => sum + item.qty,
        0,
      ),
      acceptedUnits: acceptedDeliveries.reduce(
        (sum, delivery) => sum + getDeliveryUnits(delivery),
        0,
      ),
  ...
  const warehouseOverview = useMemo(() => {
    ...
    const totalCapacity = source.reduce(
      (sum, warehouse) => sum + warehouse.capacityMax,
      0,
    );
    const usedCapacity = source.reduce(
      (sum, warehouse) => sum + warehouse.capacityUsed,
      0,
    );
    const highestWarehouse = source.reduce((highest, warehouse) => {
      return getWarehousePercent(warehouse) > getWarehousePercent(highest)
        ? warehouse
        : highest;
    }, source[0]);
    ...
    return {
      ...
      utilization:
        totalCapacity > 0
          ...
          : 0,
      highestWarehouse,
      attentionWarehouses: source.filter(
        (warehouse) => warehouse.status !== "operational",
      ).length,
      openDispatches: source.reduce(
        (sum, warehouse) => sum + warehouse.openDispatches,
        0,
      ),
    };
  }, [warehouseSites]);
  ...
  const handleReceiveStock = (e: SyntheticEvent) => {
    ...
    setTimeout(() => {
      ...
      const selectedItem = stockOptions.find(
        (s) => s.id === receiveForm.itemId,
      );
      if (!selectedItem) {
      ...
      const nextStocks = stocks.map((item) => {
        if (
          ...
          getStockOptionItemType(item) === itemType ||
      ...
      const finalStocks = nextStocks.some(
        (item) =>
          item.id === receiveForm.itemId ||
          getStockOptionItemType(item) === itemType ||
          item.name === selectedItem.name,
      ...
      const nextWarehouseStocks = selectedWarehouse
        ? (() => {
            const existing = warehouseStocks.some(
              (item) => item.id === receiveForm.itemId,
            );
            if (existing) {
            ...
            return [
              ...warehouseStocks,
              {
                ...
                stockQty: addedQty,
                location:
                  receiveForm.location || selectedWarehouse.warehouseName,
                lastMovement: new Date().toISOString().split("T")[0],
              },
            ];
          })()
      ...
      const receivedDelivery: Delivery = {
        ...
        notes: receiveForm.location
          ? `Stored at ${receiveForm.location}`
          : "Manual warehouse receipt",
  ...
  const handleDispatch = (e: SyntheticEvent) => {
    ...
    if (dispatchForm.movementType === "transfer" && !dispatchForm.toWarehouseId)
      return;
    if (dispatchForm.movementType !== "transfer" && !dispatchForm.siteName)
      return;

    setSaving(true);
    setTimeout(() => {
      const dispatchQty = Number(dispatchForm.qty);
      const itemToDispatch = stockOptions.find(
        (s) => s.id === dispatchForm.itemId,
      );

      if (!itemToDispatch || itemToDispatch.qty < dispatchQty) {
        alert(
          `Insufficient stock level. Available: ${itemToDispatch?.qty ?? 0} ${itemToDispatch?.unit ?? ""}`,
        );
        setSaving(false);
        return;
      }
      ...
      const destinationWarehouse = dispatchForm.toWarehouseId
        ? warehouseSites.find(
            (warehouse) =>
              String(warehouse.dbId) === dispatchForm.toWarehouseId,
          )
        : null;
      ...
      const nextStocks = stocks.map((item) => {
        if (
          item.id === dispatchForm.itemId ||
          getStockOptionItemType(item) === itemType ||
          item.name === itemToDispatch.name
        ) {
      ...
      const nextWarehouseStocks = selectedWarehouse
        ? warehouseStocks.map((item) => {
            ...
            return {
              ...item,
              qty: item.qty - dispatchQty,
              stockQty: Math.max(0, item.stockQty - dispatchQty),
            };
          })
        : warehouseStocks;
      ...
      const newDel: Delivery = {
        ...
        siteName:
          ...
          dispatchForm.siteName ||
          MOVEMENT_META[dispatchForm.movementType].label,
        subconName:
          destinationWarehouse?.subconName ||
          dispatchForm.subconName ||
          selectedWarehouse?.subconName ||
          "Warehouse Operations",
        status: "pending",
        date: new Date().toISOString().split("T")[0],
        items: [
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 dark:bg-zinc-950 p-6">
        <div className="max-w-md text-center bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-8 shadow-xl">
          ...
          <h2 className="mt-4 text-2xl font-black tracking-tight text-slate-800 dark:text-zinc-100">
            Access Denied
          </h2>
          <p className="mt-2 text-sm text-slate-500 dark:text-zinc-400">
            Your current account ({userRoleLabel}) does not have security
            authorization to manage warehouse inventory assets.
          </p>
          <Link
            to="/sites"
            className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-6 py-2.5 text-xs font-black text-white hover:bg-emerald-700 transition shadow"
          >
            Return to Nodes
          </Link>
        </div>
              ...
              <div className="flex items-center gap-2 rounded-2xl bg-white/50 backdrop-blur-md border border-slate-200 px-3 py-1.5 shadow-[0_18px_60px_-42px_rgba(15,23,42,0.65)] text-xs select-none dark:bg-zinc-900/50 dark:border-zinc-800">
                <div className="flex items-center gap-1.5 pr-2.5 border-r border-slate-200 dark:border-zinc-800">
                  <span className="relative flex h-2 w-2">
                    <span
                      className={`absolute inline-flex h-full w-full rounded-full opacity-75 animate-ping ${isOnline ? "bg-emerald-400" : "bg-amber-400"}`}
                    />
                    <span
                      className={`relative inline-flex h-2 w-2 rounded-full ${isOnline ? "bg-emerald-500" : "bg-amber-500"}`}
                    />
                  </span>
            ...
            <div className="relative flex-1 sm:w-64 sm:flex-initial">
              ...
              <input
                ...
                placeholder={
                  activeTab === "stocks"
                    ? selectedWarehouse
                      ? "Search materials, category, bay..."
                      : "Search warehouse, subcon, area..."
        ...
        {loading ? (
          ...
          <div className="space-y-5">
              ...
              <section className="space-y-4">
                <div className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-[0_18px_60px_-42px_rgba(15,23,42,0.65)] dark:border-zinc-800 dark:bg-zinc-900">
                  <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 px-5 py-4 dark:border-zinc-800">
                    <div className="min-w-0">
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-black text-green-700 hover:text-green-800 dark:text-green-300"
                      >
                      ...
                      <h2
                        className="mt-1 truncate text-2xl font-black tracking-tight text-slate-900 dark:text-zinc-50"
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <button
                        ...
                        onClick={() => {
                          setDispatchForm((f) => ({
                            ...f,
                            movementType: "pullout",
                            siteName: "Pull-out request",
                            subconName: selectedWarehouse.subconName,
                          }));
                      ...
                      >
                        <i className="bx bx-log-out-circle text-base" /> Request
                        Pull-out
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <i className="bx bx-transfer-alt text-base" /> Transfer
                        Warehouse
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-4">
                    {[
                      {
                        label: "Area stock units",
                        value: selectedWarehouseStats.currentUnits,
                        icon: "bx-package",
                        tone: "bg-green-50 text-green-700 ring-green-100 dark:bg-green-500/10 dark:text-green-300 dark:ring-green-900/40",
                      },
                      {
                        label: "Accepted units",
                        value: selectedWarehouseStats.acceptedUnits,
                        icon: "bx-check-circle",
                        tone: "bg-emerald-50 text-emerald-700 ring-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-300 dark:ring-emerald-900/40",
                      },
                      {
                        label: "Accepted deliveries",
                        value: selectedWarehouseStats.acceptedDeliveries,
                        icon: "bx-archive-in",
                        tone: "bg-blue-50 text-blue-700 ring-blue-100 dark:bg-blue-500/10 dark:text-blue-300 dark:ring-blue-900/40",
                      },
                      {
                        label: "Open requests",
                        value: selectedWarehouseStats.openRequests,
                        icon: "bx-loader-circle",
                        tone: "bg-amber-50 text-amber-700 ring-amber-100 dark:bg-amber-500/10 dark:text-amber-300 dark:ring-amber-900/40",
                      },
                    ].map((card) => (
                      <div
                        key={card.label}
                        className="rounded-2xl border border-slate-200/70 p-4 dark:border-zinc-800"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-2xl font-black text-slate-900 dark:text-zinc-50">
                              {card.value}
                            </p>
                          </div>
                          <div
                            className={`flex h-10 w-10 items-center justify-center rounded-2xl ring-1 ${card.tone}`}
                          >
                ...
                {filteredStocks.length > 0 && (() => {
                  const BREAKDOWN_TYPES = [
                    { key: 'cable',           label: 'Cable',            unit: 'm',   icon: 'bx-plug'             },
                    { key: 'node',            label: 'Node',             unit: 'pcs', icon: 'bx-buildings'        },
                    { key: 'amplifier',       label: 'Amplifier',        unit: 'pcs', icon: 'bx-broadcast'        },
                    { key: 'extender',        label: 'Extender',         unit: 'pcs', icon: 'bx-pulse'            },
                    { key: 'tsc',             label: 'TSC',              unit: 'pcs', icon: 'bx-box'              },
                    { key: 'powersupply',     label: 'Powersupply',      unit: 'pcs', icon: 'bx-bolt'             },
                    { key: 'powersupply_case',label: 'Powersupply Case', unit: 'pcs', icon: 'bx-cabinet'          },
                  ]
                  const totals: Record<string, number> = {}
                  for (const item of filteredStocks) {
                    const type = ('itemType' in item ? item.itemType : item.id) as string
                    const normalised =
                      type === 'psu' || type === 'power_supply' ? 'powersupply' :
                      type === 'psu_case' || type === 'ps_housing' ? 'powersupply_case' : type
                    totals[normalised] = (totals[normalised] ?? 0) + item.qty
                  }
                  return (
                    <div className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white dark:border-zinc-800 dark:bg-zinc-900 shadow-sm mb-4">
                      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 dark:border-zinc-800">
                        <div className="flex items-center gap-2">
                          <i className="bx bx-list-ul text-slate-400 text-sm" />
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Inventory · Volume Breakdown</p>
                        </div>
                        <span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-[10px] font-black text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300">
                          {filteredStocks.length} item type{filteredStocks.length !== 1 ? 's' : ''}
                        </span>
                      </div>
                      <div className="flex gap-2 p-4 overflow-x-auto">
                        {BREAKDOWN_TYPES.map(t => {
                          const qty = totals[t.key] ?? 0
                          if (qty === 0) return null
                          return (
                            <div key={t.key} className="shrink-0 overflow-hidden rounded-2xl border border-slate-200/70 bg-slate-50 dark:border-zinc-700 dark:bg-zinc-800/50 p-3 min-w-[110px]">
                              <div className="flex items-start justify-between mb-2">
                                <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 leading-tight">{t.label}</p>
                                <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-emerald-50 dark:bg-emerald-500/10">
                                  <i className={`bx ${t.icon} text-[10px] text-emerald-600 dark:text-emerald-400`} />
                                </div>
                              </div>
                              <p className="text-xl font-black font-mono leading-none text-slate-800 dark:text-slate-100">
                                {qty.toLocaleString()}
                              </p>
                              <p className="text-[9px] font-mono text-slate-400 dark:text-slate-500 mt-0.5">{t.unit}</p>
                            </div>
                          )
                        })}
                      </div>
                  ...
                  <section className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-[0_18px_60px_-42px_rgba(15,23,42,0.65)] dark:border-zinc-800 dark:bg-zinc-900">
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-zinc-800">
                      <div>
                        ...
                        <h3 className="mt-1 text-lg font-black tracking-tight text-slate-900 dark:text-zinc-50">
                          Available materials
                        </h3>
                      </div>
                    ...
                    {filteredStocks.length === 0 ? (
                      <div className="py-16 text-center text-slate-500">
                        ...
                        <p className="mt-2 text-sm font-bold">No materials found.</p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-4">
                        {filteredStocks.map((item) => {
                          ...
                          return (
                            <div key={item.id} className="rounded-xl border border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4 shadow-sm hover:shadow-md transition-shadow">
                              {/* Top row: icon + status */}
                              <div className="flex items-center justify-between mb-3">
                                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400">
                                  <i className={`bx ${item.icon} text-lg`} />
                                </div>
                                <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-wide ${isLow ? "bg-rose-50 text-rose-600 ring-1 ring-rose-200 dark:bg-rose-500/10 dark:text-rose-300" : "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300"}`}>
                                  {isLow ? "Low" : "OK"}
                                </span>
                              </div>
                              {/* Label */}
                              ...
                              {/* Total */}
                              <p className="text-2xl font-black text-slate-900 dark:text-zinc-50 leading-none">
                                {item.qty.toLocaleString()}
                                <span className="text-sm font-semibold text-slate-400 ml-1">{item.unit}</span>
                              </p>
                              {/* Footer */}
                              <div className="mt-2 flex items-center justify-between text-[10px] text-slate-400">
                                <span>/{item.threshold} threshold</span>
                    ...
                    <div className="max-h-[520px] overflow-y-auto p-4">
                      <div className="space-y-3">
                        {selectedWarehouseDeliveries.map((delivery) => {
                          ...
                          const movementMeta =
                            MOVEMENT_META[delivery.movementType];
                          return (
                            <button
                              key={delivery.id}
                              type="button"
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  ...
                                  <p className="mt-1 truncate font-black text-slate-900 dark:text-zinc-50">
                                    #{delivery.reference}
                                  </p>
                                  <p className="mt-1 truncate text-[11px] font-bold text-slate-500 dark:text-zinc-400">
                                    {delivery.source || delivery.siteName}
                                  </p>
                                </div>
                                <span
                                  className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${meta.pill}`}
                                >
                                  {meta.label}
                                </span>
                              </div>
                              <div className="mt-3 flex flex-wrap gap-1.5">
                                {delivery.items.map((item, idx) => (
                                  <span
                                    key={idx}
                                    className="inline-flex max-w-full items-center gap-1.5 rounded-md bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-600 dark:bg-zinc-800 dark:text-zinc-300"
                                  >
                        ...
                        {selectedWarehouseDeliveries.length === 0 && (
                          <div className="py-14 text-center text-slate-500">
                            ...
                            <p className="mt-2 text-sm font-bold">
                              No accepted or requested movements yet.
                            </p>
                          </div>
                        )}
                      </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {[
                {
                  label: "Subcon warehouses",
                  value: warehouseOverview.totalWarehouses,
                  icon: "bx-store",
                  tone: "bg-sky-50 text-sky-700 ring-sky-100 dark:bg-green-600/10 dark:text-sky-300 dark:ring-sky-900/40",
                },
                {
                  label: "Network utilization",
                  value: `${warehouseOverview.utilization}%`,
                  icon: "bx-pie-chart-alt-2",
                  tone: "bg-green-50 text-indigo-700 ring-indigo-100 dark:bg-green-500/10 dark:text-indigo-300 dark:ring-indigo-900/40",
                },
                {
                  label: "Max warehouse usage",
                  value: `${getWarehousePercent(warehouseOverview.highestWarehouse)}%`,
                  icon: "bx-trending-up",
                  tone: "bg-amber-50 text-amber-700 ring-amber-100 dark:bg-amber-500/10 dark:text-amber-300 dark:ring-amber-900/40",
                },
                {
                  label: "Open dispatches",
                  value: warehouseOverview.openDispatches,
                  icon: "bx-trip",
                  tone: "bg-emerald-50 text-emerald-700 ring-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-300 dark:ring-emerald-900/40",
                },
              ].map((card) => (
                <div
                  key={card.label}
                  className="rounded-2xl border border-white/70 bg-white/85 p-5 backdrop-blur-xl shadow-[0_18px_60px_-42px_rgba(15,23,42,0.65)] dark:border-zinc-800 dark:bg-zinc-900"
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">
            ...
            <div className="rounded-2xl border border-slate-200/70 bg-white shadow-[0_18px_60px_-42px_rgba(15,23,42,0.65)] dark:border-zinc-800 dark:bg-zinc-900">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-zinc-800">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-black tracking-tight text-slate-900 dark:text-zinc-50">
                    Subcontractor Warehouse Control Tower
                  </h2>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                    Highest capacity use
                  </p>
                  <p className="mt-1 text-sm font-black text-slate-800 dark:text-zinc-100">
                    {warehouseOverview.highestWarehouse.subconName} at{" "}
                    {getWarehousePercent(warehouseOverview.highestWarehouse)}%
                  </p>
                </div>
              </div>
                ...
                {filteredWarehouses.map((warehouse) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        navigate(
                          `/warehouse/${warehouseRouteSlug(warehouse)}/inventory`,
                        )
                      }
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <h3
                            className="mt-1 truncate text-base font-black text-slate-900 dark:text-zinc-50"
                            title={warehouse.subconName}
                          >
                            {warehouse.subconName}
                          </h3>
                          <p
                            className="mt-1 truncate text-xs font-bold text-slate-500 dark:text-zinc-400"
                        ...
                        <span
                          className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${meta.pill}`}
                        >
                          {meta.label}
                        </span>
                      </div>
                      ...
                      <div className="mt-5">
                        <div className="flex items-end justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-3xl font-black tracking-tight text-slate-900 dark:text-zinc-50">
                              {percent}%
                            </p>
                          </div>
                          <p className="pb-1 text-right text-[11px] font-bold text-slate-400">
                            {warehouse.capacityUsed.toLocaleString()} /{" "}
                            {warehouse.capacityMax.toLocaleString()} units
                          </p>
                        </div>
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-3 border-t border-slate-100 pt-4 text-xs dark:border-zinc-800">
                        <div>
                          ...
                          <p className="mt-1 font-black text-slate-800 dark:text-zinc-100">
                            {warehouse.materialKinds}
                          </p>
                        </div>
                        <div>
                          <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                          ...
                          <p
                            className={`mt-1 font-black ${warehouse.lowStockItems > 0 ? "text-rose-600 dark:text-rose-300" : "text-slate-800 dark:text-zinc-100"}`}
                          >
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3 text-[11px] font-bold text-slate-500 dark:text-zinc-400">
                        ...
                        <span>
                          Audited {formatDeliveryDate(warehouse.lastAudit)}
                        </span>
                      </div>
                    </div>
                  );
                ...
                {filteredWarehouses.length === 0 && (
                  <div className="col-span-full py-14 text-center">
                    ...
                    <p className="mt-2 text-sm font-bold text-slate-500">
                      No subcontractor warehouses match this search.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-4">
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {[
                {
                  label: "Open dispatches",
                  value: deliveryCounts.pending + deliveryCounts.dispatched,
                  icon: "bx-loader-circle",
                  tone: "bg-sky-50 text-sky-700 ring-sky-100 dark:bg-green-600/10 dark:text-sky-300 dark:ring-sky-900/40",
                },
                {
                  label: "In transit",
                  value: deliveryCounts.dispatched,
                  icon: "bx-trip",
                  tone: "bg-blue-50 text-blue-700 ring-blue-100 dark:bg-blue-500/10 dark:text-blue-300 dark:ring-blue-900/40",
                },
                {
                  label: "Delivered",
                  value: deliveryCounts.delivered,
                  icon: "bx-check-circle",
                  tone: "bg-emerald-50 text-emerald-700 ring-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-300 dark:ring-emerald-900/40",
                },
                {
                  label: "Units loaded",
                  value: deliveryCounts.units,
                  icon: "bx-package",
                  tone: "bg-slate-100 text-slate-700 ring-slate-200 dark:bg-zinc-800 dark:text-zinc-200 dark:ring-zinc-700",
                },
              ].map((card) => (
                <div
                  key={card.label}
              ...
              <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-[0_18px_60px_-42px_rgba(15,23,42,0.65)] dark:border-zinc-800 dark:bg-zinc-900">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-zinc-800">
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-wider">
                    {(
                      Object.keys(DELIVERY_STATUS_META) as DeliveryStatus[]
                    ).map((status) => {
                      const meta = DELIVERY_STATUS_META[status];
                      return (
                        <span
                          key={status}
                          className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 px-2.5 py-1 text-slate-500 dark:border-zinc-700 dark:text-zinc-400"
                        >
                          <span
                          ...
                          >
                            <td className="px-4 py-4">
                              <div className="flex items-center gap-3">
                                <span
                                  className={`h-10 w-1 rounded-full ${meta.dot}`}
                                />
                                <div>
                                  <p className="font-black text-slate-900 dark:text-zinc-50">
                                    #{d.id}
                                  </p>
                                  ...
                                  <span
                                    className={`mt-1 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${meta.pill}`}
                                  >
                            ...
                            <td className="px-4 py-4">
                              <p className="font-black text-slate-800 dark:text-zinc-100">
                                {movementMeta.noun}
                              </p>
                              <p
                                className="mt-1 max-w-[110px] truncate font-mono text-[11px] font-semibold text-slate-400"
                                title={d.trackingToken}
                              >
                                {d.trackingToken}
                              </p>
                            </td>
                            <td className="px-4 py-4">
                              <p
                                className="max-w-[170px] truncate font-black text-slate-800 dark:text-zinc-100"
                                title={d.siteName}
                              >
                                {d.siteName}
                              </p>
                              <p className="mt-1 text-[11px] font-semibold text-slate-400">
                                {getDeliveryUnits(d)} units scheduled
                              </p>
                            </td>
                            <td className="px-4 py-4">
                              <p
                                className="max-w-[120px] truncate font-bold text-slate-600 dark:text-zinc-300"
                                title={d.subconName}
                              >
                                {d.subconName}
                              </p>
                      ...
                      {filteredDeliveries.length === 0 && (
                        <tr>
                          ...
                          >
                            <i className="bx bx-package text-4xl text-slate-300" />
                            <p className="mt-2 text-sm font-bold">
                              No delivery logs match this search.
                            </p>
                          </td>
                        </tr>
                      )}
                ...
                {selectedDelivery && selectedDeliveryMeta ? (
                    ...
                    <div className="border-b border-slate-100 p-5 dark:border-zinc-800">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          <h3
                            className="mt-1 truncate text-xl font-black tracking-tight text-slate-900 dark:text-zinc-50"
                            title={selectedDelivery.siteName}
                          >
                            {selectedDelivery.siteName}
                          </h3>
                          <p className="mt-1 text-xs font-bold text-slate-500 dark:text-zinc-400">
                            #{selectedDelivery.reference} · Token{" "}
                            {selectedDelivery.trackingToken}
                          </p>
                        </div>
                        <span
                          className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${selectedDeliveryMeta.pill}`}
                        >
                    ...
                    <div className="space-y-4 p-5">
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
                        <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-wider text-slate-400">
                          <span>
                            {MOVEMENT_META[selectedDelivery.movementType].label}
                          </span>
                          <span className="max-w-[150px] truncate text-right">
                            {selectedDelivery.toWarehouseName ||
                              selectedDelivery.siteName}
                          </span>
                        </div>
                        <div className="relative mt-7 h-2 rounded-full bg-slate-200 dark:bg-zinc-800">
                          ...
                          <div
                            ...
                            style={{
                              left: `calc(${selectedDeliveryMarker}% - 16px)`,
                            }}
                          >
                            <i
                              className={`bx ${selectedDeliveryMeta.icon} text-lg`}
                            />
                          </div>
                        </div>
                        <div className="mt-8 flex items-start justify-between gap-3 text-[11px] font-bold text-slate-500 dark:text-zinc-400">
                          ...
                          <span className="text-center">
                            {DELIVERY_LOCATION[selectedDelivery.status]}
                          </span>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-2xl border border-slate-200 p-3 dark:border-zinc-800">
                          ...
                          <p className="mt-1 text-sm font-black text-slate-800 dark:text-zinc-100">
                            {formatDeliveryDate(selectedDelivery.date)}
                          </p>
                        </div>
                        <div className="rounded-2xl border border-slate-200 p-3 dark:border-zinc-800">
                          <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                      ...
                      <div className="rounded-2xl border border-slate-200 p-4 dark:border-zinc-800">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <h4
                              className="truncate text-sm font-black text-slate-800 dark:text-zinc-100"
                              title={selectedDelivery.subconName}
                            >
                              {selectedDelivery.subconName}
                            </h4>
                          </div>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-100 pt-3 text-[11px] dark:border-zinc-800">
                          <div>
                            <span className="block font-black uppercase tracking-wider text-slate-400">
                              From
                            </span>
                            <span className="font-bold text-slate-700 dark:text-zinc-300">
                              {selectedDelivery.fromWarehouseName ||
                                selectedWarehouse?.warehouseName ||
                                "Warehouse stock"}
                            </span>
                          </div>
                        ...
                        {[
                          {
                            ...
                            detail:
                              "Delivery request entered in the warehouse release queue.",
                            active: true,
                          },
                          {
                            label: "Gate release",
                            detail:
                              "Cargo loaded and released from the logistics gate.",
                            active: selectedDelivery.status !== "pending",
                          },
                          {
                            label: "Active transit",
                            detail:
                              "Courier team is moving toward the destination site.",
                            active: selectedDelivery.status !== "pending",
                          },
                          {
                            label: "Site receipt",
                            detail:
                              "Materials received and verified by site engineer.",
                            active: selectedDelivery.status === "delivered",
                          },
                        ].map((step) => (
                          <div key={step.label} className="flex gap-3">
                            <div
                              className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ring-4 ${step.active ? "bg-green-600 ring-green-500/15" : "bg-slate-300 ring-transparent dark:bg-zinc-700"}`}
                            />
                            <div>
                              <p
                                className={`text-xs font-black ${step.active ? "text-slate-800 dark:text-zinc-100" : "text-slate-400"}`}
                              >
                                {step.label}
                              </p>
                  ...
                  <div className="flex min-h-[420px] items-center justify-center p-6 text-center">
                    <div>
                      ...
                      <p className="mt-3 text-sm font-black text-slate-700 dark:text-zinc-200">
                        No delivery selected
                      </p>
                      <p className="mt-1 text-xs font-semibold text-slate-400">
                        Select a dispatch to view route and status details.
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-black">
                        Receive Stock Materials
                      </p>
                      <p className="mt-0.5 text-xs text-green-100">
                        Increment inventory levels
                      </p>
                    </div>
                  </div>
                  <button
              ...
              >
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setReceiveForm((f) => ({ ...f, itemId: e.target.value }))
                    }
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={saving}
                    className="flex-1 rounded-2xl bg-green-600 py-2.5 text-sm font-black text-white hover:bg-green-700 transition disabled:opacity-50"
                  >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-emerald-200">
                        Delivery, pull-out, or warehouse transfer
                      </p>
                    </div>
                  </div>
                  <button
              ...
              >
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 rounded-2xl bg-slate-100 p-1 dark:bg-zinc-900">
                    {(Object.keys(MOVEMENT_META) as MovementType[]).map(
                      (type) => {
                ...
                {dispatchForm.movementType === "transfer" ? (
                  <div>
                    ...
                    >
                      ...
                      {warehouseSites
                        .filter(
                          (warehouse) =>
                            warehouse.dbId !== undefined &&
                            (!selectedWarehouse?.dbId ||
                              warehouse.dbId !== selectedWarehouse.dbId),
                        )
                        .map((warehouse) => (
                          <option
                            key={warehouse.id}
                            value={String(warehouse.dbId)}
                          >
                            {warehouse.warehouseName} · {warehouse.subconName}
                          </option>
                        ))}
                    </select>
                  </div>
                  ...
                  <>
                    <div>
                      <label className="mb-2 block text-xs font-black text-slate-500 uppercase tracking-wider">
                        {dispatchForm.movementType === "pullout"
                          ? "Pull-out Destination / Receiver"
                      ...
                      <input
                        ...
                        placeholder={
                          dispatchForm.movementType === "pullout"
                            ? "e.g. Main warehouse return bay..."
                            : "e.g. Manila Central Site A..."
                        }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDispatchForm((f) => ({ ...f, itemId: e.target.value }))
                    }
                    className="h-10 w-full rounded-2xl border border-slate-200 bg-white px-3.5 text-sm outline-none transition focus:border-emerald-400 focus:ring-4 focus:ring-emerald-400/15 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white"
                  >
                    <option value="">-- Choose Material --</option>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDispatchForm((f) => ({ ...f, qty: e.target.value }))
                    }
                    className="h-10 w-full rounded-2xl border border-slate-200 bg-white px-3.5 text-sm outline-none transition placeholder:text-slate-300 focus:border-emerald-400 focus:ring-4 focus:ring-emerald-400/15 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setDispatchForm((f) => ({ ...f, reason: e.target.value }))
                    }
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={saving}
                    className="flex-1 rounded-2xl bg-emerald-600 py-2.5 text-sm font-black text-white hover:bg-emerald-700 transition disabled:opacity-50"
                  >
                    {saving
                      ? "Processing..."
                      : `Create ${MOVEMENT_META[dispatchForm.movementType].noun}`}
                  </button>
                </div>
              </form>
            </div>
