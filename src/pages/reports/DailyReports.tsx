...
export default function DailyReports() {
  ...
  const cachedAreas = cacheGet<Area[]>('dr_areas')
  ...
  useEffect(() => {
    const hit = cacheGet<Area[]>('dr_areas')
  ...
  function selectArea(area: Area) {
    ...
    const cacheKey = `dr_nodes_${area.id}`
  ...
  function exportSelectedSiteNodes() {
    ...
    const rows = [
      [`Site Node Report — ${selectedArea.name}`],
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'All Nodes')
    XLSX.writeFile(wb, `Site_Nodes_${safeFileName(selectedArea.name)}.xlsx`)
  }
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-5">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm font-semibold" style={{ color: BRAND.muted }}>
              {selectedArea
                ? 'Minimal and compact node monitoring table with Excel export.'
                : 'Select a site to view all nodes, recovery progress, and daily reports.'}
            </p>
          </div>
                        ...
                        <div className="relative flex min-h-[190px] flex-col justify-between">
                          <div>
                            ...
                            <p className="mt-3 text-sm font-semibold leading-6" style={{ color: BRAND.muted }}>
                              Open all nodes, cable recovery, and report details for this site.
                            </p>
                          </div>
                  ...
                  <tbody>
                    {filteredNodes.map((node, index) => {
                      ...
                      return (
                        <tr
                          key={node.id}
                          onClick={() => navigate(`/reports/daily/${node.id}`)}
                        ...
                        >
                          ...
                          >
                            <button
                              ...
                              onClick={e => {
                                e.stopPropagation()
                                navigate(`/reports/daily/${node.id}`)
                              }}
